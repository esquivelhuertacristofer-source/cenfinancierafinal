/**
 * @project CEN - Nivel 3: Planificación (Bloque 2)
 * @author Cristofer Huerta (Luminar)
 * @description Clon de alta fidelidad del Segmento 3, Bloque 2 (Hábitos y Creencias Financieras).
 */

import Link from "next/link";

export default function Segmento3Bloque2Page() {
  return (
    <main className="min-h-screen bg-white font-epilogue overflow-x-hidden">
      ...
      {/* 2. HERO DEL BLOQUE (Rojo/Coral Suave a Blanco) */}
      <section className="pt-32 pb-20 px-6 max-w-4xl mx-auto flex flex-col items-center text-center">
        <div className="bg-gradient-to-b from-[#FFE4E6] to-white w-full rounded-[4.5rem] p-12 shadow-xl border border-rose-50 relative overflow-hidden mb-12">
          <span className="bg-[#F43F5E] text-white px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest mb-4 inline-block shadow-md">
            Bloque 2
          </span>
          <h1 className="text-4xl md:text-5xl font-black text-[#BE123C] tracking-tighter mb-6">
            Hábitos y Creencias <br/> Financieras
          </h1>
          <p className="text-slate-500 font-bold text-sm leading-relaxed max-w-2xl mx-auto">
            Descubre qué es un hábito financiero y cómo las creencias que tenemos desde pequeños pueden estancar nuestro crecimiento. Aprende a reprogramar tu mente para el éxito.
          </p>
        </div>

        <h3 className="text-[#F43F5E] font-black text-xl mb-6 tracking-tighter">En Este Bloque Aprenderemos</h3>
        <div className="flex flex-wrap justify-center gap-4 mb-16">
           <span className="bg-[#FFF1F2] text-[#E11D48] border border-[#FDA4AF] px-6 py-3 rounded-full font-black text-xs uppercase tracking-widest shadow-sm">Los Hábitos</span>
           <span className="bg-[#FFF1F2] text-[#E11D48] border border-[#FDA4AF] px-6 py-3 rounded-full font-black text-xs uppercase tracking-widest shadow-sm">Las Creencias</span>
           <span className="bg-[#FFF1F2] text-[#E11D48] border border-[#FDA4AF] px-6 py-3 rounded-full font-black text-xs uppercase tracking-widest shadow-sm">El Cambio</span>
        </div>
      </section>

      {/* 3. SECCIÓN DEL VIDEO LARGO */}
      <section className="max-w-6xl mx-auto px-6 pb-20 text-center">
        <h2 className="text-4xl font-black text-[#FFCA28] tracking-tighter mb-10 drop-shadow-sm" style={{ WebkitTextStroke: '1px #F43F5E' }}>
        ...
        <div className="w-full aspect-video bg-slate-100 rounded-[4.5rem] shadow-2xl overflow-hidden flex items-center justify-center border-8 border-slate-50 relative">
           <div className="absolute inset-0 flex items-center justify-center flex-col">
              <div className="w-20 h-20 bg-[#F43F5E] rounded-full flex items-center justify-center shadow-lg shadow-rose-500/50 mb-4 pl-2 cursor-pointer hover:scale-110 transition-transform">
      ...
      <div className="w-full text-center pb-12 bg-white">
        <h2 className="text-3xl md:text-4xl font-black text-[#FFCA28] tracking-tighter leading-tight drop-shadow-sm" style={{ WebkitTextStroke: '1px #F43F5E' }}>
            ¡Nos vemos en el próximo <br/> video para una historia <br/> muy especial!
        </h2>
      </div>

      {/* 4. ZONA ROJA VIBRANTE: JUEGOS Y RECURSOS */}
      <section className="w-full bg-[#F43F5E] py-24 px-6 relative">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
           
           <h2 className="text-3xl md:text-4xl font-black text-[#FFCA28] tracking-tighter mb-4">
              ¿Qué tan sano es tu bolsillo?
           </h2>
           <p className="text-white font-bold text-sm mb-12">
              Conoce las cosas que guían tu comportamiento financiero.
           </p>

           <div className="w-48 h-48 bg-white/20 rounded-[3rem] flex items-center justify-center border-4 border-dashed border-white/40 text-white/50 font-bold mb-16 shadow-xl hover:-translate-y-2 transition-transform">
              [ IMG CENy Hábitos ]
           </div>

           {/* Juego 1 (Caja Amarilla/Naranja) */}
           <div className="w-full bg-[#F59E0B] rounded-[4.5rem] p-8 shadow-2xl hover:-translate-y-2 transition-transform border-4 border-white/20 mb-24">
              <h3 className="text-2xl font-black text-[#011C40] tracking-tighter mb-2 mt-2">Desafía Tus Conocimientos Con Un Quiz Financiero!</h3>
              
              <div className="w-full aspect-[4/3] md:aspect-[21/9] bg-white rounded-[3rem] flex flex-col items-center justify-center text-slate-300 font-black text-xl mb-6 border-4 border-dashed border-orange-200 mt-8">
                 <span className="text-[#0980E8] text-sm tracking-widest uppercase mb-2 block">Quiz Interactivo</span>
                 <span className="text-[#011C40] text-2xl">El Espejo de Mis Finanzas</span>
           ...
           <div className="w-full flex flex-col items-center">
              ...
              <div className="w-48 h-48 bg-white/20 rounded-full flex items-center justify-center border-4 border-dashed border-white/40 text-white/50 font-black mb-8 shadow-lg">
                 [ IMG Tarro de Monedas ]
              </div>

              <h2 className="text-3xl md:text-4xl font-black text-white tracking-tighter mb-8 drop-shadow-sm">
                 La historia de este bloque es: <br/> <span className="text-[#FFCA28]">La Grieta Invisible: De cómo la imagen pasó a Recordar!</span>
              </h2>
              ...
              <div className="w-full max-w-2xl bg-white/20 backdrop-blur-md rounded-full p-4 flex items-center gap-4 border border-white/40 shadow-xl mb-20">
                 <div className="w-12 h-12 bg-[#FFCA28] rounded-full flex items-center justify-center shrink-0 cursor-pointer hover:scale-105 transition-transform">
                    <div className="w-0 h-0 border-t-[8px] border-t-transparent border-l-[12px] border-l-[#E11D48] border-b-[8px] border-b-transparent ml-1"></div>
                 </div>
                 <div className="flex-1 h-2 bg-white/30 rounded-full relative">
                    <div className="absolute top-0 left-0 h-full w-2/3 bg-[#FFCA28] rounded-full"></div>
                 </div>
                 <span className="text-white font-black text-xs tracking-widest">05:40 / 08:15</span>
              </div>
              ...
              <h3 className="text-2xl font-black text-white tracking-tighter mb-8 drop-shadow-sm">Guía de Bloque (Auditoría para Recordar)</h3>
      ...
      <section className="w-full bg-[#0980E8] pt-24 pb-20 px-6 border-t-4 border-white/20">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
            <h2 className="text-3xl md:text-5xl font-black text-[#FFCA28] tracking-tighter mb-6 drop-shadow-lg">
              ¡Has Llegado Muy Lejos! <br/>
              <span className="text-white">Es Hora de Convertir Tu Conocimiento en Un Proyecto Real</span>
            </h2>
            <p className="text-white/80 font-bold uppercase tracking-widest text-xs mb-12 max-w-xl">
              ¿Has prestado atención a la lección sobre tus hábitos? Descarga estas plantillas, enfréntate a tus verdaderas creencias y descubre que eres un verdadero experto!
            </p>
            ...
            <div className="flex flex-col md:flex-row justify-center gap-6 w-full mb-16">
               <div className="bg-white rounded-[2.5rem] p-8 flex-1 shadow-2xl hover:-translate-y-2 transition-transform cursor-pointer border-4 border-transparent hover:border-white/50">
                  ...
                  <h3 className="font-black text-[#011C40] mb-2 uppercase tracking-widest text-sm">Diario de Hábitos</h3>
               </div>
               <div className="bg-white rounded-[2.5rem] p-8 flex-1 shadow-2xl hover:-translate-y-2 transition-transform cursor-pointer border-4 border-transparent hover:border-white/50">
                  <div className="w-16 h-16 bg-[#0980E8]/10 text-[#0980E8] rounded-full flex items-center justify-center mx-auto mb-6 font-black text-2xl">↓</div>
                  <h3 className="font-black text-[#011C40] mb-2 uppercase tracking-widest text-sm">Mapa de Creencias</h3>
               </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 w-full justify-center">
               ...
               {/* Conectamos al Bloque 3 */}
               <Link href="/academia/planificacion/bloque3">
      ...
      {/* 6. ACCESO A BIBLIOTECA (Footer en Rojo/Coral Suave) */}
      <section className="w-full bg-[#FB7185] pt-16 pb-32 px-6 border-t border-white/20">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-center gap-16">
           <div className="text-white md:w-1/2 text-center md:text-left">
              <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tighter leading-tight">
                 Acceso a la <br/> <span className="text-[#BE123C]/80">biblioteca</span>
              </h2>
              <p className="font-bold text-sm text-[#BE123C] leading-relaxed mb-8 max-w-sm mx-auto md:mx-0">
              ...
              <button className="bg-[#BE123C] text-white px-8 py-4 rounded-full font-black shadow-xl hover:scale-105 transition-all uppercase text-xs">
                 Accede Ahora
              </button>
           </div>
